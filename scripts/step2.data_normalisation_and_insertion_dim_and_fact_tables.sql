/*
=======================================================
Script: step2.data_normalisation_and_insertion_dim_and_fact_tables.sql
Description: Normalize raw data and create dimension and fact tables
Purpose: Extract distinct dimension records and persist them in staging tables
Author: Data Engineer Andile Dube
Date: 2026-05-14
=======================================================
*/

-- Dimensional table loading from raw PC data
-- Location dimension
SELECT
     DISTINCT [Continent],
     [Country_or_State],
     [Province_or_City]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_location]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Priority dimension
SELECT
     DISTINCT [Priority]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_priority]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Payment dimension
SELECT
     DISTINCT [Payment_Method]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_payment]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Store dimension
SELECT
     DISTINCT [Shop_Name],
     [Shop_Age]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_store]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Date dimension
SELECT
     DISTINCT [Purchase_Date],
     [Ship_Date]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_date]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Customer dimension
SELECT
     DISTINCT [Customer_Name],
     [Customer_Surname],
     [Customer_Contact_Number],
     [Customer_Email_Address],
     [Sales_Person_Name],
     [Sales_Person_Department]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_customer]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- PC specification dimension
SELECT
     DISTINCT [PC_Make],
     [PC_Model],
     [Storage_Type],
     [RAM],
     [Storage_Capacity]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_pc_spec]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Channel dimension
SELECT
     DISTINCT [Channel]
INTO [PC_Sales_Staging_dtw].[dbo].[dim_channel]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];

-- Fact table loading from raw PC data
SELECT
     DISTINCT [Cost_Price],
     [Sale_Price],
     [Discount_Amount],
     [Finance_Amount],
     [Credit_Score],
     [Cost_of_Repairs],
     [Total_Sales_per_Employee],
     [PC_Market_Price]
INTO [PC_Sales_Staging_dtw].[dbo].[Fact_pc_sales]
FROM
     [PC_Sales_Staging_dtw].[dbo].[Raw_PC_Data];